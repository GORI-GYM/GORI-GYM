...
function HistoryBadge({ entry }: { entry: DailyMissionHistoryEntry }) {
  ...
  return (
    <div className={`rounded-xl border px-3 py-2 text-center text-sm font-bold ${className}`}>
      <div>{label}</div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-[430px] bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-white"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-white font-bold text-lg">ミッション設定</h3>
          <button type="button" onClick={onClose} className="border border-[#F5A623] text-[#F5A623] rounded-lg px-4 py-2">閉じる</button>
        </div>
        ...
        <div className="space-y-4">
          {bodyParts.map((bodyPart) => {
            ...
            return (
              <div key={bodyPart} className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4">
                <div className="mb-3 text-white font-bold text-lg">{getBodyPartLabel(bodyPart)}</div>
          ...
          <div className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4">
            <div className="mb-3 text-white font-bold text-lg">自分の種目を追加</div>
            <div className="grid grid-cols-[1fr_auto] gap-2">
              <input
                value={customName}
                onChange={(event) => setCustomName(event.target.value)}
                placeholder="例: インクラインダンベルプレス"
                className="bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-[#ccc] outline-none"
              />
              <select
            ...
            <button
              ...
              className="mt-3 w-full bg-[#F5A623] text-black font-bold rounded-lg px-4 py-2"
            >
              種目を追加
            </button>
          </div>
          ...
          <button
            ...
            className="w-full bg-[#F5A623] text-black font-bold rounded-lg px-4 py-2"
          >
            保存する
          </button>
        </div>
...
export default function DailyMissionSection({
  ...
  return (
    <>
      <section className="bg-[#0a0a0a] space-y-6 px-4 pb-4 pt-2">
        <div
          className="relative overflow-hidden bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-white space-y-6"
          style={{
            backgroundImage:
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[#888] text-sm">Today&apos;s Mission</div>
                <h2 className="mt-2 text-white font-bold text-lg">{getMissionHeadline(day)}</h2>
            ...
            {day.isRestDay ? (
              <div className="mt-5 bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4">
                <div className="text-white font-bold text-lg">今日は休息日！ゴリラも一緒に回復中🦍</div>
                <div className="mt-2 text-[#ccc]">ストレッチ、水分補給、睡眠で明日のミッションに備えよう。</div>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <div className={`inline-flex rounded-lg px-4 py-2 text-sm font-bold bg-[#F5A623] text-black ${theme.badge}`}>
                          {getBodyPartLabel(mission.bodyPart)}
                        </div>
                        <div className="mt-3 text-white font-bold text-lg">{mission.exerciseName}</div>
                        <div className="mt-1 text-[#ccc]">{getMissionDescription(mission)}</div>
                        <div className="mt-3 bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-[#888] text-sm">
                          推奨重量: <span className="font-bold text-white">{mission.recommendedWeight}</span>
                        </div>
                      </div>
                      <button
                        type="button"
                        ...
                        className={`min-w-[92px] rounded-lg px-4 py-2 text-sm transition ${
                          mission.completed
                            ? "bg-[#F5A623] text-black font-bold"
                            : "border border-[#F5A623] text-[#F5A623]"
                        }`}
            ...
            {day.completedAll && !day.isRestDay ? (
              <motion.div
                ...
                className="mt-5 bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-center text-white"
              >
                <div className="text-[#888] text-sm">Mission Complete!</div>
                <div className="mt-2 text-white font-bold text-lg">MISSION COMPLETE!</div>
                <div className="mt-2 text-[#ccc]">今日のミッションをすべて達成した！</div>
              </motion.div>
            ) : null}

            <div className="mt-5">
              <div className="mb-3 text-[#888] text-sm">7 Days History</div>
      ...
      <AnimatePresence>
        {showCompleteFx ? (
          <motion.div
            ...
            className="pointer-events-none fixed inset-x-0 top-24 z-50 mx-auto w-fit bg-[#1a1a1a] rounded-xl border border-[#2a2a2a] p-4 text-white font-bold text-lg"
          >
            +{DAILY_MISSION_XP}XP
          </motion.div>
        ) : null}
